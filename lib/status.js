...
Status.prototype.setup = function (cb) {
	...
	var findAppProcessList = function (next) {
		...
		that.findProcessList(null, function (error, list) {
			if (error) {
				throw error;
			}
			processList = list;
	...
	var findSockFile = function (next) {
		...
		if (that.isRunning) {
			...
			// get pids
			var pids = [];
			return that.getPids(processList, null, function (error, list) {
				if (error) {
...
Status.prototype.getStatus = function (cb) {
	...
	var onData = function (data) {
		...
		that.findProcessList(data.msg.exec, function (error, list) {
			if (error) {
				return that.end(error);
			}
			that.getPids(list, data.msg.exec, function (error, processes) {
				if (error) {
					return that.end(error);
				}
...
Status.prototype.outputStatus = function (data, processes) {
	...
	var now = Date.now();
	var sinceStart = ((now - started.getTime()) / 1000 / 60 / 60).toFixed(2);
	var sinceReload = ((now - reloaded.getTime()) / 1000 / 60 / 60).toFixed(2);
	msgList.push('Application started at                   : ' +  started);
	if (data.msg.reloadedCount) {
		msgList.push('Application reloaded at          : ' + reloaded);
	}
	msgList.push('Application running hours since start    : ' + sinceStart + ' hours');
	if (data.msg.reloadedCount) {
		msgList.push('Application running hours since reload   : ' + sinceReload + ' hours');
	}
...
Status.prototype.findProcessList = function (execPath, cb) {
	...
	exec(command, function (error, stdout) {
		...
		var appPath = (execPath || process.execPath) + ' ' + path;
...
Status.prototype.getPids = function (processList, execPath, cb) {
	var target = 'node';

	if (execPath) {
		target = '"(' + target + '|' + execPath + ')"';
	}	
				...
				if (processList[i].indexOf(pid) !== -1) {
					res.push({
						process: processList[i].substring(
							processList[i].indexOf((execPath || process.execPath))
						),
						pid: parseInt(pid, 10)
					});
				}
...
Status.prototype.running = function (cb, counter, running, seen) {
	...
	counter += 1;
	this.findProcessList(null, function (error, processList) {
		if (error) {
			return cb(error);
		}
		// we found some process(es) running
		if (processList && processList.length) {
			that.getPids(processList, null, function (error, list) {
				var len = list.length;
				for (var i = 0; i < len; i++) {
...
Status.prototype.notRunning = function (cb, counter) {
	...
	this.findProcessList(null, function (error, list) {
		if (error) {
			return cb(error);
		}
