...
Status.prototype.outputStatus = function (data, processes) {
	...
	msgList.push('Daemon name' + space(30) + ': ' + print.g(data.msg.name));
	...
	msgList.push('Application' + space(30) + ': ' + print.p(data.app));
	...
	for (var i = 0, len = sortedList.length; i < len; i++) {
		...
		if (p.indexOf('monitor') === -1) {
			...
			// index 1 is monitor
			// last index is pid
			app = sep[0] + ' ' + sep[1] + ' ' + sep[sep.length - 1];
		}
		msgList.push(prefix + tree + print.b(app));
	}
	
	msgList.push('Logging path' + space(29) + ': ' + (logPath || print.r('Not logging')));

	if (data.msg && data.msg.watching) {
		msgList.push(
			'Watch for changes (auto-restart/reload)  : ' +
			data.msg.watching.join(' ')
		);
	}
