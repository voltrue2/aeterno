...
function outputHelpText() {
	...
	print.out((help.config || '') + (help.configDesc || ''));
	...
	print.out((help.forced || '') + (help.forcedDesc || ''));
	...
	print.out((help.update || '') + (help.updateDesc || ''));
}
...
function execCommand(name, cmd) {
	...
	switch (name) {
		...
		case args.COMMANDS.update:
		case args.COMMANDS.status:
		case args.COMMANDS.tail:
		...
		case args.COMMANDS.stopall:
		case args.COMMANDS.restartall:
			cmd();
			break;
		default:
			print.error(print.r('No valid command to run: ' + '"' + name + '"'));
			break;
	}
}
...
function handleAddingOptions(path, cb) {
	...
	var addwatch = args.getWatchList();
	var taskList = [];
	
	if (addlog) {
		taskList.push('addlog');
	}

	if (addwatch.length) {
		taskList.push('addwatch');
	}
...
function getPath() {
	...
	// if path is just index.js leave it as is
	if (path === 'index.js') {
		return path;
	}
